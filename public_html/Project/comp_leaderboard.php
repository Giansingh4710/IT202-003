...
$db=getDB();
$info=$db->prepare(
    "SELECT username,(Select IFNULL(SUM(Correct),0) from ScoreHistory 
    WHERE ScoreHistory.user_id=Users.id AND ScoreHistory.created >= Competitions.created AND ScoreHistory.created<= Competitions.expires) AS sum  
